...
class File extends Model
{
    ...
    {
        return $this->belongsTo(User::class, 'uploading_user_id', 'id')->withDefault();
    }
    ...
    public function scopeSearchName($query, $search = '%')
    {
        return $query->where('name', 'like', "%{$search}%")
            ->orWhere(function ($query) use ($search) {
                $query->where('uploading_user_id', User::where('first_name', $search)->value('id'));
            })
            ->orWhere(function ($query) use ($search) {
                $query->where('uploading_user_id', User::where('last_name', $search)->value('id'));
            });
    }
