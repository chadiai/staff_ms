{{--View for people that aren't logged in--}}
@guest
        ...
        <x-tmk.section>
            ...
            <div class="bg-gray-200 bg-opacity-25 grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 p-6 lg:p-8">
                    <div class="">
                        ...
                        <p class="mt-4 text-gray-600 text-md leading-relaxed">
                    ...
                    <div class="pt-4 md:pt-2 lg:pt-16 pb-4">
                        ...
                        <p class="mt-4 text-gray-600 text-md leading-relaxed mb-4">
...
@auth()
        <x-slot name="description">dashboard</x-slot>
        <x-slot name="title">
            {{ auth()->check() ? auth()->user()->first_name . "'s dashboard" : "Dashboard" }}
        </x-slot>
        ...
        <x-tmk.section>
            {{-- tasks for users--}}
                <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
                    ...
                    <div class="p-2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mt-3">
                            @if ($groupedTasks && $groupedTasks > 0)
                                {{--Display a card for each day the user has task(s)--}}
                            @foreach($groupedTasks as $date => $group)
                                ...
                                <div style="min-height: 100px;">
                                    <a class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow h-full">
                                        ...
                                        <ul class="pb-1 pl-6 list-disc inline-block">
                                            {{--Display all the tasks with the location (if not empty) inside the card--}}
                                            @foreach($group as $task)
                                                <li wire:key="task-{{ $task->id }}" class="pb-4 hover:font-semibold" data-tippy-content="Click for more info" wire:click="setDescription({{ $task->id }})">
                                                    '{{ $task->name}}'@if(!empty($task->location))
                                                        at {{ $task->location }}
                                                    @endif
                                                </li>
                                            @endforeach
                    ...
                    @endif
                    {{--Appointments--}}
                    @if(auth()->user()->isAdminOrSuperAdmin() || auth()->user()->isGrandson() )
                    <div class="p-2">
                        <h1 class="text-xl font-medium text-gray-900">
                                    ...
                                    <a class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow h-full">
                                        ...
                                        <ul class="pb-1 pl-6 list-disc inline-block">
                                            ...
                                            @foreach($group as $appointment)
                                                <li class="pb-4 hover:font-semibold" data-tippy-content="Click for more info" wire:click="setDescription({{ $appointment->id }})" >
                                                    '{{ $appointment->name}}'@if(!empty($appointment->location))
                ...
                <hr>
            {{--Links to different pages with explanation--}}
            <div class="bg-gray-200 bg-opacity-25 grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 p-6 lg:p-8">
                    ...
                    @endif
                    {{-- Superadmin, admin and cook can only plan a meal--}}
                    @if (auth()->user()->isAdminOrSuperAdmin() || auth()->user()->isCook())
                        <div>
                            <div class="flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" class="ml-1 w-5 h-5 fill-black">
                    ...
                    {{--Only superadmins and admins can mangage users and view paperwork--}}
            ...
            {{--Modal to show more information about an event--}}
                <x-dialog-modal id="descriptionModal"
                                wire:model="showDescriptionModal">

                    <x-slot name="title">
                        @if($currentEvent and !$showModal)
                            <h1 class="text-3xl">{{$currentEvent->name}}</h1>
                            <div class="ml-auto">
                                <span>{{ $currentEvent->start_date_format }} - {{ $currentEvent->end_date_format }}</span>
                            </div>
                        @endif
                    </x-slot>
                    <x-slot name="content">
                        ...
                        @if($currentEvent and !$showModal)
                            ...
                            <div class="mt-1 p-2 text-xl">{{$currentEvent->description}}</div>
                            <h2 class="font-bold text-xl block mt-2 w-full">Members</h2>
                            @if($showDescriptionModal)
                                <x-tmk.list title="Members">
                                    {{--Show all the task members assigned to the event--}}
                                    @foreach ($eventMembers as $member)
                                        <li class="mt-1 text-xl">{{$member->first_name}} {{$member->last_name}}</li>
                                    @endforeach
                                </x-tmk.list>
                            @endif
                            @isset($currentEvent->location)
                                <h2 class="font-bold text-xl block mt-2 w-full">Location</h2>
                                <div class="mt-1 p-2 text-xl">{{$currentEvent->location}}</div>
                            @endisset
                        @endif
                    </x-slot>
                    <x-slot name="footer">
                        <x-secondary-button wire:click="resetDescription()">Cancel</x-secondary-button>
